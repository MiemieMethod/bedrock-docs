...
class ChatDbManager(object):
	...
	def InsertChatRecord(self, fromUid, toUid, message, currentTime, callback):
		'''
		向聊天记录数据库，插入一条聊天记录，无论是否成功，都会调用参数中的回调函数（失败仅记录错误日志）
		完整的处理逻辑：
		1、向聊天记录数据库，插入一条聊天记录
		2、插入成功后，判定是否此两人的私聊记录是否已经读取到内存中了
		3、假如尚未读取，那么从聊天记录数据库，读取此两人的私聊记录到内存中
		4、把新的一条聊天记录，保存到内存缓存中
		5、调用参数中的回调函数
		'''
	...
	def DeleteChatRecord(self, fromUid, toUid, callback = None):
		"""
		删除和某个好友的聊天记录，同时删除内存中缓存的对应聊天记录，一般是删除好友的时候发生，无论是否成功，都会调用参数中的回调函数（失败仅记录错误日志）
		"""
	...
	def InsertChatRecordInCache(self, fromUid, toUid, message, chatIndex):
		"""
		向内存缓存中插入一条聊天记录（lobby/game用），假如对应聊天的双方的聊天记录还没有数据库读取到内存缓存，那么就先从数据库读取对应的聊天记录到缓存中
		"""
	...
	def PopChatRecord(self, fromUid, toUid):
		"""
		从内存缓存中删除一条最久远的聊天记录（可用于聊天记录的总数限制）
		"""
	...
	def QueryChatRecord(self, selfUid, friendUid, callback, useCache = True):
		"""
		查询指定两人之间的聊天历史记录，可指定是否优先从内存缓存中读取，实现的功能有：
		1、假如愿意优先从内存缓存中读取聊天历史消息，且内存缓存中存在对应的信息，直接调用参数中的回调函数返回
		2、否则向数据库查询指定两人之间的聊天历史记录
		3、查询返回后，更新内存缓存中的聊天历史记录
		4、调用参数中的回调函数返回符合需求的聊天历史记录
		"""
		uida = selfUid
		uidb = friendUid
		...
		mysqlPool.AsyncQueryWithOrderKey(str([selfUid, friendUid]), sql, sqlparam, lambda args: QueryChatRecordCb(args, callback))
