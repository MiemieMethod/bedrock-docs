...
class MyGuildScreen(ScreenNode):
	...
	def ReturnLobby(self, args):
		touchEvent = args["TouchEvent"]
		touch_event_enum = clientApi.GetMinecraftEnum().TouchEvent
		if touchEvent == touch_event_enum.TouchUp:
			data = {'playerId': clientApi.GetLocalPlayerId()}
			guildConsts.GetClientModSystem().NotifyToServer(guildConsts.ReturnLobbyFromClientEvent, data)
	...
	def Create(self):
		...
		self.AddTouchEventHandler(self.mReturnBtn, self.ReturnLobby)
