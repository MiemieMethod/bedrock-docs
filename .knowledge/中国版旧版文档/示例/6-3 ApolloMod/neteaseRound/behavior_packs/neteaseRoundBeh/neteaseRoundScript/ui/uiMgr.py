...
from neteaseRoundScript.roundConst import ModName
import neteaseRoundScript.ui.uiDef as uiDef
import weakref
...
class UIMgr(object):
	...
	def InitSingleUI(self, uiKey, config):
		...
		clientApi.RegisterUI(ModName, uiKey, cls, screen)
		extraParam = {}
		if config.has_key("isHud"):
			extraParam["isHud"] = config["isHud"]
		ui = clientApi.CreateUI(ModName, uiKey, extraParam)
		if not ui:
	...
	def RegisterUIOpen(self, uiKey):
		...
		#clientApi.SetInputEnable(False)
