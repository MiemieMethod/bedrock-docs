...
def ChangeEntityPos(entityId, pos):
	global residenceSystem
	comp = residenceSystem.CreateComponent(entityId, "Minecraft", "pos")
	return comp.SetPos(tuple(pos))

def ChangePlayerDimension(playerId, dim, pos):
	global residenceSystem
	comp = residenceSystem.CreateComponent(playerId, "Minecraft", "dimension")
	return comp.ChangePlayerDimension(dim, tuple(pos))

def NotifyOneMessage(playerId, message):
	global residenceSystem
	comp = residenceSystem.CreateComponent(playerId, "Minecraft", "msg")
	comp.NotifyOneMessage(playerId, message, "")
...
def LoadModConf():
	...
	mainConfig = commonNetgameApi.GetModJsonConfig("neteaseResidenceScript")
	typeDefineConfig = mainConfig.get("typeDefineConfig", {})
	_modConf = typeDefineConfig.get("default", {})
	gameType = commonNetgameApi.GetServerType()
	_modConf.update(typeDefineConfig.get(gameType, {}))
