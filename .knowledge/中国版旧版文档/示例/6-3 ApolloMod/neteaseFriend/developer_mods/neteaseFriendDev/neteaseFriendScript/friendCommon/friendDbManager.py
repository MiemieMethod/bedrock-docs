...
class FriendDbManager(object):
	...
	def ServerPrePareData(self, uid, callback, params):
		def getTemp(a, params):
			self.ServerGetTemp(uid, False, callback, params)
		def getUnRead(a,params):
			self.ServerGetUnread(uid, False, getTemp, params)
	...
	def ServerGetTemp(self, uid, useCache, callback, params):
		'''
		lobby/game 未读,lobby/game是直接从db取数据，不管现在有什么
		'''
		logout.info("ServerGetTemp", uid, useCache, callback, params)
		if useCache:
			friend = self.getFriendData(uid)
			if friend and friend.GetTempFriends() is not None:
				callback(friend.GetTempFriends(), params)
				return
		sql = 'SELECT * From neteaseFriendTemp WHERE uid = %s'
		sqlparam = (uid,)
		logout.info("ServerGetTemp", uid, useCache, callback, params)
		mysqlPool.AsyncQueryWithOrderKey(str(uid), sql, sqlparam,
										 lambda args: self.ServerGetTempCallBack(uid, args, callback, params))

	def ServerGetTempCallBack(self, uid, args, callback, params):
		logout.info("ServerGetTempCallBack", uid, args, callback, params)
		if self.getFriendData(uid) is None:
			friend = friendData.CFriendData()
			friend.SetUid(uid)
			self.mFriends[uid] = friend
		...
		for data in args:
			if data:
				# uid = data[0] #mysql读出来是元组的形式
				fuid = data[1]
				temp_list.append(fuid)
		self.mFriends[uid].SetTempFriend(temp_list)
		logout.info("ServerGetTempCallBack", self.mFriends[uid].GetTempFriends())
		callback(self.mFriends[uid].GetTempFriends(), params)
