...
class GuildMgrGac(object):
	...
	def OnAddPlayer(self, args):
		...
		self.mGuildConsumeDiamond = args.get('guildConsumeDiamond', 0)
		self.SetMyGuildUI()
		self.SetCoinLabel()
	...
	def OnShowTips(self, args):
		...
		def ClickSureCancelCB():
			...
			if code == guildConsts.CodeCreateSuccess:
				self.OnShowGuild()
	...
	def SetMyGuildUI(self):
		...
		print guildOne.GetAttr(GuildAttrType.ApplicationQueue) != None and len(guildOne.GetAttr(GuildAttrType.ApplicationQueue)) > 0
	...
	def SetCoinLabel(self):
		"""
		刷新代币数量
		"""
		UICreate = guildConsts.GetClientModSystem().GetUIMgr().GetUI(UIDef.UI_CREATEGUILD)
		UICreate.SetCoinLabel(self.mGuildConsumeDiamond)
		
	def OnShowCreateGuildUIFromServer(self, args):
		"""
		显示创建公会界面
		"""
		enoughDia = args.get("enoughDiamond", True)
		UICreate = guildConsts.GetClientModSystem().GetUIMgr().GetUI(UIDef.UI_CREATEGUILD)
		UICreate.SetCoinColor(enoughDia)
	...
	def OnShowGuild(self):
		...
		if playerOne:
			...
			if playerAtGuildId != -1 and self.mGuildDict.get(playerAtGuildId, None) != None:
				...
				data = {'uid': playerUid}
				guildConsts.GetClientModSystem().NotifyToServer(guildConsts.GetGuildBriefFromClientEvent, data)
				UIJoin = guildConsts.GetClientModSystem().GetUIMgr().GetUI(UIDef.UI_JOIN)
				UIJoin.ShowPanel(True)
