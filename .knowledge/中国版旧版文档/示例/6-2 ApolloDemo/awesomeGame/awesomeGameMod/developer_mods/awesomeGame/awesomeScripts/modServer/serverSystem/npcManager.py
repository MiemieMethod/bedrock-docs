...
class NpcManager(object):
	...
	def _CreateSingleNpc(self, id):
		...
		exist = chunkComp.CheckChunkState(0, npc_data.pos)
		if (not exist) or (int(exist) == -1):
			print 'create npc failed'
			return False
		# 创建npc，并设置属性。
		temp_entity = self.server.CreateTempEntity()
		type_comp = self.server.CreateComponent(temp_entity.mId, 'Minecraft', 'type')
		type_comp.type = serverApi.GetMinecraftEnum().EntityConst.TYPE_NPC
		engine_type = self.server.CreateComponent(temp_entity.mId, 'Minecraft', 'engineType')
		engine_type.engineType = serverApi.GetMinecraftEnum().EntityType.Husk
		pos_comp = self.server.CreateComponent(temp_entity.mId, 'Minecraft', 'pos')
		pos_comp.pos = (npc_data.pos[0], npc_data.pos[1], npc_data.pos[2])
		rot_comp = self.server.CreateComponent(temp_entity.mId, 'Minecraft', 'rot')
		rot_comp.rot = (0, 180)
		dimesion_comp = self.server.CreateComponent(temp_entity.mId, 'Minecraft', 'dimension')
		dimesion_comp.dimensionId = 0
		name_comp = self.server.CreateComponent(temp_entity.mId, 'Minecraft', 'name')
