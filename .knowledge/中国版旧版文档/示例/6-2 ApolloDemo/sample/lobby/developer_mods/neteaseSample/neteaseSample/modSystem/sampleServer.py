...
class SampleServer(ServerSystem):
	...
	def Destroy(self):
		logout.info('--------systemServer====destroy!!!!!~~~~~')
	...
	def OnNpcTouched(self, args):
		...
		if not npc_data:
			logout.warning("no npc_data")
			return
		args["damage"] = 0
		if npc_data.speckey == "ranking":
			logout.info("show showRanking")
			data = self.CreateEventData()
			...
			logout.info("show showMenuPanel")
			data = self.CreateEventData()
	...
	def OnMenuChooseOk(self, args):
		logout.info("OnMenuChooseOk %s" % str(args))
	...
	def send2master_matching(self, playerId, is_start):
		...
		if not player_data:
			logout.warning("send2master_matching by not player")
			return
		netgame_uid = player_data.netgame_uid
		if not netgame_uid:
			logout.warning("send2master_matching fail netgame_uid=%s" % str(netgame_uid))
			return
		data = self.CreateEventData()
		data["id"] = netgame_uid
		data["serverId"] = netgame_api.get_netgame_serverid()
		...
		logout.info("send2master_matching done")
	...
	def OnPlayerMatchingStatus(self, args):
		...
		if not player_data:
			logout.warning("change matching status by no player")
			return
		data = self.CreateEventData()
	...
	def OnPlayerMatchingResult(self, args):
		...
		player_data = self.find_player_by_netgame_uid(args["id"])
		if not player_data:
			logout.warning("receive matching result by no player")
			return
		data = self.CreateEventData()
		data['suc'] = args['suc']
		if data['suc']:
	...
	def OnPlayerMatchSucRec(self, args):
		...
		if not player_data:
			logout.warning("receive client match rec by not player")
			return
		netgame_api.transfer_to_other_server_by_id(args["id"], args["serverid"])
		logout.info("OnPlayerMatchSucRec done")
		logout.info("==================begin trans2idv")
