...
def read_redis_config():
	...
	print "read_redis_config", exist, host, port, password
...
def read_mysql_config():
	...
	print "read_mysql_config", exist, host, user, password, database, port
...
def mysql_query_playerdata(idx, playerkey):
	...
	db = get_mysql_db()
	if not db:
		print "mysql_query_playerdata fail by no db"
		return idx, False, ""
	cursor = db.cursor()
	...
	db.close()
	print "mysql_query_playerdata finish"
	return idx, suc, data
...
def mysql_save_playerdata(idx, playerkey, playerdata):
	...
	db = get_mysql_db()
	if not db:
		print "mysql_save_playerdata fail by no db"
		return idx, False
	cursor = db.cursor()
	...
	try:
		...
		suc = False
	db.close()
	print "mysql_save_playerdata finish"
	return idx, suc
#-----------------------------------------------------------------------------------------------
# 从redis读取玩家二进制字符串存档
def redis_query_playerdata(idx, playerkey):
	db = get_redis_db()
	if db is None:
		print "redis_query_playerdata fail by no db"
		return idx, False, ""
	#
	...
	if not data:
		...
		suc = True
	print "redis_query_playerdata finish"
	return idx, suc, data
...
def redis_save_playerdata(idx, playerkey, playerdata):
	...
	if not suc:
		...
		suc = True
	print "redis_save_playerdata finish"
	return idx, suc
