...
/// note | Android存储位置
Android版只有在设置中的文件存储位置为“外部”时，世界目录才通常能够通过文件管理器访问。若文件存储位置为“应用程序”，世界会保存在应用数据内，备份或迁移时应优先使用游戏内导出功能。
///
...
- **游戏规则**：各项游戏规则的当前设置。
- **实验性开关**：各项实验性玩法的启用状态。若需要手动编辑，请参阅[开启实验性玩法](../../help/minecraft/enabling-experimental-features.md)。
...
玩家数据存储在LevelDB数据库中。本地玩家的数据以`~local_player`为键存储；其余玩家的数据则以`player_<客户端标识符>`形式的键单独存储，并配合记录玩家标识到存储键映射关系的键。即便`level.dat`的根标签中存在玩家数据，`~local_player`键中的数据也会优先于它。
