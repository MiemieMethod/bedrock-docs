...
附着物定义文件位于资源包的`attachables/`目录中。最新版Microsoft Learn仍将`minecraft:attachable`作为正式文档化定义，并继续记录其`item`、`scripts.pre_animation`、`scripts.parent_setup`等字段。
...
较新的官方文档还记录了可选的`description.item`字段，用于把附着物显式关联到一个或多个物品条件；若未指定，通常回退到以`identifier`匹配。
