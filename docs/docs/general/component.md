...
附加包系统中，部分JSON对象结构在多个组件和系统中通用。以下是常见的公共结构。

### 范围对象

**范围对象（Range Object）**用于定义两个数值之间的区间，结构如下：
...
使用范围对象时，系统会在最小值（含）和最大值（含）之间随机选取一个数值。每次求值都会独立随机取值。最大值不可小于最小值，但允许两者相等以实现固定取值。

### 分数对象

**分数对象（Fraction Object）**通过分子与分母定义分数关系，结构如下：
...
该对象在求值时计算 `numerator ÷ denominator`。分子和分母的值必须均大于等于`1`，且分母不能等于分子（即不能定义值为1的分数）。
