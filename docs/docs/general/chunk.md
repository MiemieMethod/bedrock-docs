...
**常加载区域（Ticking Area）**是一种无论玩家是否在附近都保持加载和模拟的区块区域。常加载区域通过`/tickingarea`命令创建，用于使特定区域的游戏逻辑（如红石装置、作物生长、实体行为等）在无玩家在场时仍持续运行。

每个存档可创建的常加载区域数量有上限，默认为10个。每个常加载区域的大小上限为100×100个区块。
