...
外显版本一般指Minecraft基岩版在各个平台商店与安装文件中显示的版本名称，例如
...
**构建版本（Build Version）**是每次游戏构建时生成的一个版本号，用于标识当前构建的版本，通常是一个递增的整数。不同平台、不同架构、不同构建类型（零售构建与开发构建）乃至服务端和客户端的构建都共享同一套构建版本，每次构建时会进行递增。
