...
**骨架模型（Skeleton Model）**是中国版中独有的模型种类，在中国版文档中也被称为**骨骼模型（**英文同上**）**。骨架模型使用JSON格式定义，可以通过附加包进行修改，但一般而言，更倾向于使用工具直接生成。骨架模型能够支持挂接中国版特效、挂接中国版序列帧动画并在纸娃娃状态下播放动画，但不支持作为方块模型和附着物模型。
