...
Endstone文档站当前仅保留`stable`与`latest`两个版本入口，说明其文档以稳定版和最新开发版两条线维护。实际部署时仍应把Endstone版本与BDS版本成组核对。

## 版本节奏与兼容策略

Endstone文档站当前采用`stable`与`latest`双入口。前者面向可部署的稳定版本，后者面向最新开发状态。这意味着实际使用时，不应把某个教程中的示例版本号长期视为通用答案，而应在升级前重新核对当前发行说明中的目标BDS版本与API变更。
...
| Endstone DevTools | 用于从原版服务端导出数据的开发工具。当前可见构建主要面向Windows，并要求系统具备OpenGL功能。 |
