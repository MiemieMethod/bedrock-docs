...
若需要按官方站点入口、常见问题与版本表的原始脉络阅读，可先查看[LeviLamina文档归档](../../translations/levilamina/index.md)。

本页用于说明LeviLamina“是什么”；若需要阅读安装、构建与发布等实践路径，应继续查看[LeviLamina入门](../../guides/servers/levilamina.md)与[LeviLamina开发者指南](../../guides/servers/levilamina/index.md)。翻译归档中的页面则用于保留官方原始资料脉络，不替代站内教程。

## 概述

LeviLamina是LiteLoaderBDS的继任项目。其官方资料将它描述为轻量级、模块化、多功能的基岩版模组加载器，目标是为基岩版提供必要的模组API和开发基础设施。与以高层抽象为主的插件API不同，LeviLamina更接近原生模组开发框架：开发者通常使用C++编写模组，并在需要时访问或拦截基岩版引擎内部对象。若需要追溯项目起源和更名缘由，可继续查看[常见问题](../../translations/levilamina/faq.md)。

LeviLamina文档也包含客户端安装与客户端构建相关内容。客户端侧能力属于LeviLamina客户端模组生态，不代表普通基岩版客户端原生支持加载第三方C++模组。
...
LeviLamina官方资料将许可与使用边界拆分在许可证、EULA、使用指南与更新日志等多份文件中。LeviLamina仓库的非闭源部分按LGPL-3.0发行，但LeviMC的闭源软件另有单独的[最终用户许可协议](../../translations/levilamina/eula.md)；官方[使用指南](../../translations/levilamina/usage-guidelines.md)则进一步约束名称、品牌、资产和免责声明的使用方式。

这意味着，LeviLamina既不是单纯只有一份许可证文本的开源项目，也不适合只凭Docker环境变量或安装命令推断全部法律边界。若需要按文件分工追溯这些资料，可直接阅读[法律与发布说明](../../translations/levilamina/legal-release-notes.md)与[LeviLamina文档归档](../../translations/levilamina/index.md)。
...
模组包通常由`tooth.json`描述包名、版本、依赖、资源文件和放置位置，并可通过Bedrinth等模组包索引分发。对服主而言，LeviLamina是随BDS版本一起维护的运行时；对开发者而言，它也是一套原生C++模组框架。

## 发布与变更

LeviLamina的发布说明不仅记录新功能，也经常记录运行时数据升级、头文件同步、崩溃修复和安全相关修复。对于服主和模组开发者而言，[更新日志](../../translations/levilamina/changelog.md)与GitHub发布页并不是附属材料，而是判断某个版本是否值得升级、是否存在已知风险的重要依据。

官方更新日志中已经出现过`SubChunkRequestPacket`漏洞修复、`Certificate::validate`崩溃修复，以及围绕`MCPE-228407`的兼容处理。这类条目说明，LeviLamina版本差异不仅影响API可用性，也可能直接影响服务器稳定性和网络安全性。
...
- 官方版本表、安装命令和支持范围会随项目发布变化，应以LeviLamina当前文档和发行页为准。

## 进一步阅读

- [文档首页](../../translations/levilamina/official-docs-home.md)
...
- [项目构建指南](../../guides/servers/levilamina/build-guide.md)
- [官方构建指南](../../translations/levilamina/build-guide.md)
