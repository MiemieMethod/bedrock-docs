...
值得注意的是，BDS在较近的版本中不再随服务端发布符号（即映射表），使基于符号注入和静态分析的BDS-Based方案的开发和维护难度大幅上升，导致部分此类项目停止维护，社区生态中自实现服务端的比例因此有所提升。
...
PocketMine-MP是使用PHP语言实现的基岩版服务端软件，是基岩版较早形成规模的第三方服务端路线之一。它拥有成熟的PHP插件生态、独立的`plugin.yml`清单模型，以及围绕`.phar`插件包形成的分发方式。关于其定位、配置文件和插件模型，见[PocketMine-MP](pocketmine-mp.md)。
