...
LeviLamina是目前基岩版原生插件生态中活跃的插件加载器之一，它基于BDS运行，通过钩子和公共API拦截或扩展游戏的原生函数。LeviLamina的原生模组通常使用C++编写，见[LeviLamina](levilamina.md)。
...
插件的分发方式因加载器而异。大多数插件通过GitHub、论坛或专用的插件市场进行分发。LeviLamina生态使用lip包管理器用于模组安装和版本管理；SerenityJS当前工作区则把插件系统拆分为独立的`@serenityjs/plugins`包。