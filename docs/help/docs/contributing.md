...
HTML块是一种将Markdown内容包裹在任意HTML元素中的内容块。使用`html`作为内容块类型，并在`|`后指定标签名。类、ID和其他特性可以用类似CSS选择器的语法直接附加在标签名后。例如：
...
默认情况下，HTML块会根据标签名自动判断内容的处理方式：`div`等块级元素的内容会按块级Markdown解析，`span`等行内元素的内容会按行内Markdown解析，`pre`等标签的内容则视为原始文本。如果需要手动指定，可以使用`markdown`选项。可用的模式有`block`、`inline`、`raw`、`auto`和`html`。例如，强制`pre`按块级Markdown解析：
...
更多的功能可以参考[HTML扩展文档](https://facelessuser.github.io/pymdown-extensions/extensions/blocks/plugins/html/)。

#### 定义列表

定义列表通过`define`内容块来创建。将术语作为段落，定义作为列表项。例如：
...
更多的功能可以参考[Definition扩展文档](https://facelessuser.github.io/pymdown-extensions/extensions/blocks/plugins/definition/)。

#### 标签组

标签组通过连续的`tab`内容块来创建，每个`tab`需要在`|`后指定标签标题。连续的标签块会自动归为一组。例如：
...
如果需要在两个紧邻的标签组之间强制断开，可以使用`new`选项：
...
更多的功能可以参考[Tab扩展文档](https://facelessuser.github.io/pymdown-extensions/extensions/blocks/plugins/tab/)。

#### 图注表注

图注和表注通过`caption`、`figure-caption`和`table-caption`内容块来创建。将`caption`块紧跟在需要添加注释的内容（如图片或表格）之后即可。`figure-caption`和`table-caption`会自动添加带有编号的前缀（如"Figure 1."和"Table 1."）并自动生成锚点ID。例如：

/// tab | 通用图注
```markdown
![placeholder](https://dummyimage.com/600x200/eee/aaa)
...
默认图注添加在内容下方。若需要将图注置于内容上方，可以在`|`后使用`<`标记：
...
更多的功能可以参考[Caption扩展文档](https://facelessuser.github.io/pymdown-extensions/extensions/blocks/plugins/caption/)。
...
本站使用MathJax渲染数学公式，支持LaTeX语法。公式分为行内公式和块级公式。
...
图片可以通过[注入](#注入特性)实现左右对齐、设置宽度和懒加载等，具体详见[Material for MkDocs图片参考文档](https://squidfunk.github.io/mkdocs-material/reference/images/#usage)。此外，在本站，如果使用图注，请使用[图注块](#图注表注)来添加图注，而不是按照前者参考文档指导的方法进行添加。

### 按钮

通过[注入](#注入特性)`.md-button`类，可以将任何链接渲染为按钮。添加`.md-button--primary`类可以将其渲染为实心按钮。按钮也支持添加图标。例如：

```markdown
[普通按钮](#){ .md-button }

[实心按钮](#){ .md-button .md-button--primary }

[带图标按钮 :material-arrow-right:](#){ .md-button }
```
...
脚注用于在不打断文档流的情况下添加补充信息。脚注引用使用`[^标识符]`语法，标识符可以是任意字符串。脚注内容在文档中任意位置声明，但始终渲染在页面底部。例如：

```markdown
这是一段带有脚注的文本[^1]，脚注也可以使用文字标识符[^note]。
...
网格用于将内容以卡片或矩形方式排列。网格需要在`<div>`标签中指定`class="grid"`（和可选的`cards`）和`markdown`特性。你也可以用HTML块快速实现`<div>`标签。

#### 卡片网格

卡片网格将每个列表项包裹为可悬浮的卡片。使用`grid`和`cards`两个类：

```markdown
/// html | div.grid.cards
- :material-file-document: __文档__ 用于记录内容
- :material-code-braces: __代码__ 用于实现功能
- :material-palette: __样式__ 用于美化界面
///
```
...
通用网格允许将任意块级元素排列在矩形中。只需使用`grid`类包裹内容即可：
...
工具提示允许为链接和其他元素添加悬浮提示信息。链接的工具提示使用标准Markdown的标题语法即可：

```markdown
[悬浮查看提示](https://example.com "这是一个工具提示！")
```
...
对于其他元素，可以通过[注入](#注入特性)`title`特性来添加工具提示：
...
此外，本站支持缩写词。缩写词的定义类似脚注，以`*`开头，所有匹配的文本在页面中都会显示为带有工具提示的虚线下划线。本站已通过全局缩写词表自动为所有页面添加了通用缩写词，详见<!-- md:samp includes/abbreviations.md -->文件。如果需要在页面中局部添加，语法如下：
...
| 独有性 | `<!-/- md:flag china -->`<br/>`<!-/- md:flag edu -->`<br/>`<!-/- md:flag vanilla -->`<br/>`<!-/- md:flag dev -->`  | <!-- md:flag china --><br/><!-- md:flag edu --><br/><!-- md:flag vanilla --><br/><!-- md:flag dev -->    |
...
换言之，`samp`的作用是显示语义等宽文本而非字面等宽文本，例如：<!-- md:samp docs -->文件夹的名称是`docs`，而不是`typewriter_text`。
