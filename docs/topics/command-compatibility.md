...
但还需要继续确认它究竟已经进入哪个发布通道。例如，`26.10.25`预览版命令表已经出现`camera attach_to_entity`、`camera detach_from_entity`和`camera play_spline`，而`26.1.1`正式版命令表中尚未出现这三条语法。若教程直接把预览版命令写成“当前版本可用”，玩家在正式版中就会得到错误结论。
