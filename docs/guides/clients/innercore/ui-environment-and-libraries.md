...
Nernar维护的InnerCore文档还包含旧界面系统、区块源、粒子、维度、库和变更记录资料。本页只整理正文较完整的部分，并明确说明资料不足之处。
...
常见操作包括放置方块、读取方块ID、读取方块状态、设置额外方块、生成实体和查询区域。例子使用`setBlock`、`getBlockId`、`getBlock`和`setExtraBlock`演示了这些能力。
...
InnerCoreUI系统以窗口、背景绘制和元素为核心。元素写在窗口描述对象的`elements`字段中，每个元素有类型、坐标和交互处理函数。
...
由于缺少完整英文维度教程，本页不提供自定义维度API步骤。需要维护旧项目时，应回到原项目代码和对应InnerCore版本的API参考核对。
