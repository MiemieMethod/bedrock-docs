...
这是一篇面向旧版中国版MC Studio特效编辑器项目维护的归档说明。它整理旧版特效编辑器模型导入、粒子特效、序列帧特效、模型挂接、预览面板、第一人称模型和属性字段；这些内容只适用于旧版中国版工具链，不表示国际版粒子系统或当前中国版特效系统具有相同格式。
...
旧版`emittertype`的示例默认值曾写作`Spere`，而参数范围写作`Sphere`。维护旧文件时应把这类拼写视为旧版资料或工具输出的历史细节，不能直接推广为当前字段拼写规则。
