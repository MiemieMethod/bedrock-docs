...
2. 再看[附加包开发工作流](addon-development-workflow.md)，建立“规划—实现—测试—发布”的统一流程。
3. 完成[制作纹理包](creating-texture-packs.md)，确认游戏能加载你的资源包。
4. 学习[本地化](localization.md)，把游戏内显示名称从硬编码改成语言键。
5. 进入[数据驱动](data-driven/index.md)，开始创建实体、方块、物品、配方和世界生成内容。
6. 学习[JSON UI进阶技巧](json-ui/index.md)，掌握HUD改造、绑定技巧和界面兼容实践。<!-- md:flag vanilla -->
7. 最后学习函数、世界模板、皮肤包和结构包，让附加包更适合发布或复用。

遇到问题时，参考[附加包故障排除](troubleshooting.md)。

准备发布前，建议再过一遍[创作者方针与发布规范](creator-guidelines.md)，把目录清理、路径长度、命令频率和UI改动风险这些“后期最容易踩坑”的点一次性处理掉。
