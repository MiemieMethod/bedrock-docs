...
这篇教程只做“能落地验证的最小练习”。目前知识库里的国际版维度JSON资料主要来自自动导出参考，存在根对象命名不一致和字段说明缺失的问题，所以这里不把它当作稳定生产方案。
...
/// note | 生成器取值仍缺资料
知识库中缺少`generator_type`的官方完整取值清单。本教程无法给出“所有版本都可用”的枚举表，实际项目请在目标版本中逐项验证。
///
