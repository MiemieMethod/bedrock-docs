...
2. [迁移旧版数据驱动文件](version-migration.md)：处理旧字段迁移、组件改名和版本升级后的兼容问题。
3. [制作模型](creating-models.md)：学会用Blockbench导出几何体和纹理。
4. [认识Molang](understanding-molang.md)：理解动画、粒子和渲染条件中常见的表达式。
...
- [自定义地物与生成技巧](adding-custom-features.md)：先认识地物、地物规则和基础生成组合。
- [生成自定义矿石](generating-custom-ores.md)：学习矿石地物的基础写法与高度分布控制。
- [生成地表斑块](surface-patches.md)：学习如何混合多个单方块地物形成地表装饰斑块。
- [通过结构模板地物生成结构](generating-structures.md)：学习把`.mcstructure`放进世界。
- [利用方块条件控制地物放置](feature-block-conditions.md)：学习借助哑地物实现条件放置。
- [使用噪声高度图生成地形](heightmap-noise.md)：学习用噪声与散植地物构造连续地形。
