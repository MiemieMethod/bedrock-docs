...
中国版旧版资料中的做法，是先把按钮、准心、面板背景等图片导入资源管理器，再新建界面文件。常见资源会进入资源包的`ui/`与贴图目录中；保存界面后，编辑器会导出可直接运行的JSON界面文件。
...
旧版资料中，继承`common.base_screen`的画布会引入额外根节点路径，用来处理安全区域和异形屏适配。这样做的副作用是：控件在脚本里访问时，路径可能不再只是`/panel/text`这种相对短路径，而要额外拼接一段基础路径。
