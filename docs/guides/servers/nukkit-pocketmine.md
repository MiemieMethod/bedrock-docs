...
知识库中的服务端软件列表把PocketMine-MP列为活跃的PHP基岩版服务端项目，也列出了NetherGamesMC/PocketMine-MP等相关项目。但本次处理的Nukkit-MOT文档没有提供PocketMine-MP的安装教程、插件开发教程或API参考。因此，本站暂时不编写PocketMine-MP分步教程，后续需要补充可靠资料后再扩展。
