...
Allay要求Java21运行。常见选择是使用GraalVM获得较好的性能，或者使用OpenJDK获得更稳定、熟悉的环境。安装后先在终端确认版本：
...
更省事的做法是使用AllayLauncher下载、更新和管理服务器。Windows可以在PowerShell中运行安装脚本：
...
| `logs/server.log` | 当前服务器日志文件。 |
| `logs/*.log.gz` | 按日期与序号轮转后的压缩日志。 |
...
Allay插件可以使用Java、Kotlin、Scala以及其他JVM语言编写。入门时通常从模板项目开始会更省事：
...
Allay插件主类继承`Plugin`。插件通常会经历加载、启用和禁用阶段：
...
Terra插件可以作为Allay的世界生成扩展使用。下载`Terra-allay-<version>-shaded.jar`后，把它移动到Allay的`plugins`目录。然后编辑`worlds/world-settings.yml`，把目标维度的生成器改为`TERRA`：
