...
/// warning | 平台限制
知识库中的官方README将编辑器描述为面向Windows电脑、键盘和鼠标的Preview工作流，不适用于移动端或主机。若后续官方页面调整入口，也应先确认当前来源文档对应的启动路径，而不要假定所有版本都共享同一入口。
///
...
安装完成后，再进入编辑器。知识库中的编辑器README给出的最短路径是：
...
如果你想做桌面快捷方式，可以创建一个Windows快捷方式。知识库中的不同官方材料对协议写法存在版本差异，常见写法包括以下两种：
...
如果你希望按知识库中的官方仓库README走最短路径，可以直接按这三步执行：
...
另外，如果你做的是“编辑器外”的普通脚本测试，记得在实验性玩法中开启Beta APIs开关；这是知识库中的官方README给出的前置提醒。
