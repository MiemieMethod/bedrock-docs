...
BetterRTX是一个面向Minecraft基岩版Windows10/11版RTX渲染器的第三方社区模组。它的公开介绍把它定位为面向基岩版的光线追踪着色器模组，目标是在官方RTX实现现有能力的基础上继续修复与增强画面表现。
...
从现有公开介绍来看，它采用的是“反编译—修改—重新编译—再注入”的着色器改造流程。公开列出的代表性改动包括：
...
BetterRTX公开说明中提到，测试覆盖了NVIDIA、Intel和AMD显卡，并以尽量维持性能为目标；但这类结论仍然只是项目方对其测试环境的描述，不能替代目标玩家设备上的实际验证。

另外，公开说明还提到：由于制作过程可能涉及Minecraft原始着色器代码，仓库没有公开完整源代码，而是只公开功能说明、示意代码片段与效果对比图。这意味着：
...
- 可以写“BetterRTX公开介绍中提到某项渲染改进”。
- 不宜写“安装后必然生效”“所有RTX设备都支持”或“这是基岩版官方渲染能力”。

现有公开信息已经足够说明BetterRTX的大致定位、实现方式和代表性特性；但还不足以支撑稳定的版本对应表、安装教程或分发承诺。因此，本页仍以辨析边界和测试方法为主，而不把BetterRTX写成官方开发主线。