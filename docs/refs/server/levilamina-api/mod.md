...
- [Config（配置系统）](config.md) — 使用 `getConfigDir()` 获取配置文件位置
- [Data（数据结构）](data.md) — 使用 `getDataDir()` 存储数据库
- [I18n（国际化）](i18n.md) — 使用 `getLangDir()` 存储翻译文件
- [IO & Logger（日志系统）](io.md) — `getLogger()` 返回模组的日志器
