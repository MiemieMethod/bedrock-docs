...
| JavaScript插件旧资料 | 旧资料提到其可通过销毁并重建运行上下文实现热重载，但应以当前版本实际能力为准。 |
...
Allay旧开发资料还补充了方块与物品标识符的关系：原版注册过程中同时存在“纯物品”“实际方块物品”和“硬方块物品”。当某个方块已经有同ID的实际方块物品时，冲突的硬方块物品会以`item.`前缀注册，例如类似`minecraft:item.cake`的路径。Allay希望把已注册方块的硬方块物品ID尽量与方块ID保持一致；如果方块存在额外的实际方块物品，则`BlockType#getItemType()`返回的是实际方块物品，而不是带`item.`前缀的硬方块物品。
...
Allay较新的资料还提供了按维度管理的绘图形状接口，可把简单几何体直接发送给客户端用于调试或可视化。
...
形状通常通过`Dimension#addPrimitiveShape(...)`添加到维度中，再通过`removePrimitiveShape(...)`或`removeAllPrimitiveShapes()`移除。资料说明，玩家进入对应维度后会看到这些形状，离开维度时会自动失去可见性；也可以通过`addViewer(...)`和`removeViewer(...)`只对指定玩家显示。