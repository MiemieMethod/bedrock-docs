...
/// warning | 版本提示
Allay的服务端版本、API版本和Minecraft基岩版协议版本相互关联。具体接口签名、类名和可用性会随版本变化，使用前应先核对当前发行版、Javadoc和运行时行为。
///
...
加载流程中，Allay会先遍历插件源，再为每个插件文件选择加载器、读取描述符并检查循环依赖，最后调用`onLoad()`。因此，不应把游戏内初始化写进`onLoad()`，否则即使表面上“能运行”，也会让插件生命周期变得脆弱且难以重载。

## 热重载

| 情况 | 说明 |
|------|------|
| `Plugin.isReloadable()`返回`false` | 默认状态，JAR插件不参与热重载。 |
| 仅返回`true`但未实现`reload()` | 控制台会给出警告；这不表示插件已经真正支持热重载。 |
| JAR插件调用`reload()` | 只会执行插件自身的重载逻辑，不会自动重新从JAR文件读取新代码。 |
...
实现`Permissible`的对象可以执行权限检查。Allay默认玩家权限计算器会让操作员拥有全部权限，让非操作员只拥有有限权限。插件也可以替换`PermissionCalculator`实现自定义权限系统。

## 事件

事件监听方法使用`@EventHandler`标注，并且应只有一个事件类型参数和`void`返回值。监听器需要注册到事件总线。

Allay存在服务器实例事件总线和世界事件总线。世界相关事件应注册到对应世界的事件总线；非世界事件由服务器实例事件总线处理。
...
| `BlockState` | 某一方块种类的具体不可变状态。方块状态通常可以按单例使用`==`比较。 |
...
方块类型和物品类型之间存在对应关系，但并非所有方块物品都与方块标识符一一同名。处理方块转物品时，应以实际注册结果为准。
...
伪容器用于创建自定义GUI。通常应使用`addPlayer()`显示给玩家，因为该方法会在打开容器前处理客户端所需的伪方块。

容器还支持打开、关闭和槽位变化监听，适合商店、菜单和状态同步界面。
...
Boss栏对象在修改属性后会自动把更新同步给当前查看者，但不会替插件自动清理查看者列表。
...
常见入口包括`I18n.get().tr(...)`和`player.sendTranslatable(...)`。当目标语言缺失某个翻译键时，系统会回退到`en_US`。

## 持久化数据容器

**持久化数据容器（Persistent Data Container）**，简称PDC，用于在Allay对象上保存插件自定义数据。常见持有者包括世界、实体、方块实体和物品栈。
...
如果原生类型不够用，还可以自行实现`PersistentDataType`，把复杂对象映射成受支持的基础类型再存储。
...
| `renderOffsets.scale(...)`与`renderOffsets.textureSize(...)` | 辅助方法 | 用于快速调整较大贴图或特殊手持缩放。 |

生成器会按物品组件自动识别盔甲、工具、食物和有耐久物品，并写入相应客户端属性。自定义物品仍需要资源包提供对应纹理。

## 调试形状

Allay还提供了按维度管理的绘图形状接口，可把简单几何体直接发送给客户端用于调试或可视化。
...
形状通常通过`Dimension#addPrimitiveShape(...)`添加到维度中，再通过`removePrimitiveShape(...)`或`removeAllPrimitiveShapes()`移除。玩家进入对应维度后会看到这些形状，离开维度时会自动失去可见性；也可以通过`addViewer(...)`和`removeViewer(...)`只对指定玩家显示。