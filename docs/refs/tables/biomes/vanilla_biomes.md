...
> 文档版本：1.26.20-beta.24

/// abstract | 描述
本页列出原版生物群系的完整[赋命名空间标识符]与对应数值ID。数值ID用于网络协议、存档数据及旧版接口的兼容识别，附加包通常使用字符串标识符来引用生物群系。

  [赋命名空间标识符]: ../../../../docs/concepts/identifier.md
///

/// note | 关于数值ID的连续性
原版生物群系的数值ID并不连续，部分数值区间留空以供历史上已废弃的生物群系占位，或因版本演进而产生间隔。表中仅列出当前存在的生物群系。
///
