...
本页列出方块描述`menu_category.group`字段和物品描述`menu_category.group`字段所接受的创造模式分组（物品组）字符串枚举值。
...
| `itemGroup.name.element` | `construction` | 元素（仅教育版）<!-- md:flag edu --> |
...
| `itemGroup.name.chemistrytable` | `items` | 化学台（仅教育版）<!-- md:flag edu --> |
