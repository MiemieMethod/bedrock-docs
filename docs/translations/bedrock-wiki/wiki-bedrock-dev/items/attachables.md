...
/// tip
本文件假设你对Molang、渲染控制器、动画和客户端实体定义有基本了解。请确保你熟悉[客户端实体](../entities/entity-intro-rp.md)的基础知识！
///
...
-   列出了材料和纹理用于附魔闪光。如果你的物品在附魔时应该有闪光效果，这一点非常重要。
...
在第一种方法中，我们将使用玩家的骨架副本构建附加物，将你的模型附加到玩家的一个骨骼上。
...
使用文本编辑器，从提供的玩家骨架文件中复制骨骼到你的几何体文件中，然后将`rightItem`骨骼设置为你模型中立方体的父骨骼。将此几何体保存到你的资源包中。
...
让你的模型漂浮在玩家的脚下并不是理想的。我们的下一步是创建动画，以便能够正确显示模型。

创建两个新动画，一个用于第一人称持有物品，另一个用于第三人称持有物品。选择你的第三人称动画，并根据需要进行定位。将此动画保存到你的资源包中。
...
:::tip
要为玩家的手添加动画，你需要使用玩家的动画，而不是附加物的动画。
:::
...
/// warning | 注意
这时事情变得复杂。两个动画需要同时播放；你的第一人称动画和引导的第一人称动画。

确保在进行更改时正在编辑你的动画。首先选择它，然后在上面播放引导的第一人称动画。
///
...
-   将你模型的根骨骼设置为玩家骨架中的`rightItem`骨骼的子骨骼。
...
确保在进行更改时正在编辑你的动画。首先选择它，然后在上面播放引导动画。
///

播放两个动画，并根据需要定位你的模型。将动画保存到你的资源包中。
...
与第三人称动画类似，请查看附加物引导文件并将`wiki.first_person_guide`动画导入Blockbench。将你的动画与引导的第一人称动画一起播放，然后进行更改并保存文件。

## 示例包

这两种方法已被编译成一个示例包，供你参考，以防你遇到困难或只是想查看一个工作示例。
