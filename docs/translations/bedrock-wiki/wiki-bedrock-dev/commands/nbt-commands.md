...
锁定库存中的任何适用槽位: `/give @p iron_axe 1 100 {"minecraft:item_lock":{ "mode": "lock_in_inventory" }}`

锁定库存中的特定槽位: `/give @p apple 1 0 {"minecraft:item_lock":{ "mode": "lock_in_slot" }}`
...
物品将在实体死亡时保留在其库存中: `/replaceitem entity @e[type=zombie] slot.weapon.mainhand -1 cooked_beef 1 0 {"minecraft:keep_on_death":{}}`

对于非玩家实体，物品在死亡后不会出现在其库存中，因为它们不会重生。可以使用 `/clear` 或 `/replaceitem` 从实体的库存中清除或替换这些物品。

> 游戏规则 /keepinventory 设置为 true 时，效果类似于玩家库存中的所有物品都具有 `"minecraft:keep_on_death":{}`。然而，NBT 组件在筛选特定物品以使其保留在玩家库存中而非所有物品时最为有用。

KeepOnDeath 在冒险模式和生存模式下的功能完全相同。

## 组合 NBT 组件

给所有玩家发放一把锁定在特定库存槽位并在死亡时保留的弓: `/give @a bow 1 0 {"minecraft:item_lock":{ "mode": "lock_in_slot" }, "minecraft:keep_on_death":{}}`

给自己发放一把只能挖掘砂砾和沙子的石铲，并锁定在库存中: `/give @s stone_shovel 1 0 {"minecraft:can_destroy":{"blocks":["dirt", "sand"]},"minecraft:item_lock":{ "mode": "lock_in_inventory" }}`
