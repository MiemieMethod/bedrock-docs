...
本教程假设您对方块和脚本编写有高级理解。
在开始之前，请先查看[方块指南](/blocks/blocks-intro)。
:::

本教程将指导您制作具有子基数旋转的方块（例如苦力怕头颅和标志），并提供具有此旋转类型的“壳”方块示例。

_寻找常规旋转？请[点击这里](/blocks/rotatable-blocks)了解更多！_
...
我们将通过使用[自定义组件](/blocks/block-events)和更具体地说，[beforeOnPlayerPlace](/blocks/block-events#before-player-place)钩子，在方块放置之前更新我们的方块状态。这意味着在我们的事件处理程序中，我们可以访问玩家的旋转。
...
然后，使用[排列](/blocks/block-permutations)来定义基础基数旋转，这将通过我们模型中的精确骨骼进行扩展。
