...
/// tip
重要的是要记住，`runtime_identifier` 仅会解析实体的硬编码属性。这意味着使用100%数据驱动的生物作为运行时标识符不会为你的实体添加任何新属性。此外，一些实体的运行时可能会覆盖在组件部分中找到的属性，即使它们已被添加，例如Shulker实体的碰撞盒大小。
///
...
-   使实体在不在水中时翻滚。
-   与水桶互动将给予你一桶鳕鱼，但当桶中的鳕鱼被放置时，将放置桶装实体而不是鳕鱼。
-   赋予实体特殊的游泳和重力行为。
...
-   为投射物实体添加面朝玩家的动画。
-   使实体破坏。
-   使用其生成蛋生成此类实体时，它将在玩家位置生成，而不是你放置的位置。它还会朝向天空。
...
-   使实体在不在水中时翻滚。
-   与水桶互动将给予你一桶河豚，但当桶中的河豚被放置时，将放置桶装实体而不是河豚。
-   赋予实体特殊的游泳和重力行为。
...
-   使实体在不在水中时翻滚。
-   与水桶互动将给予你一桶鲑鱼，但当桶中的鲑鱼被放置时，将放置桶装实体而不是鲑鱼。
-   赋予实体特殊的游泳和重力行为。
...
-   使实体在不在水中时翻滚。
-   赋予实体特殊的游泳和重力行为。
-   当右键点击水桶时，会给予你一桶热带鱼。如果实体没有任何 `minecraft:variant`、`minecraft:mark_variant`、`minecraft:color` 和 `minecraft:color2`，则将名称设置为白色Kob，这可能是热带鱼的0变体。如果应用了一个或多个上述组件，则将桶的名称更改为其他名称（并且该桶放置的是实体，而不是带有该名称/数据值的热带鱼）。
