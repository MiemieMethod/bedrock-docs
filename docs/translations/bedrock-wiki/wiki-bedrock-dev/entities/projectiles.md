...
## 附加信息
在创建自定义投射物（例如箭矢或三叉戟变体，或完全自定义的投射物）时，你可能需要考虑定义一个 [runtime identifier](../entities/runtime-identifier.md)，以确保其按预期运作。如果不这样做，可能会导致不期望的行为，从奇怪的视觉效果到不正确的击退方向，以及用徒手就能杀死的箭矢。