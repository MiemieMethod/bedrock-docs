...
```
如前所述，我们可以定义多个纹理。如果我们想要不同变体的实体，这将非常有用，比如上面的蜜蜂。此外，我们可以使用多个纹理在不同的基础上叠加不同的纹理，就像村民有不同的生物群系基础和不同的职业层次一样。你可以查看我们关于渲染控制器的页面[这里](../entities/render-controllers.md)，了解有关如何叠加纹理的更多细节。
