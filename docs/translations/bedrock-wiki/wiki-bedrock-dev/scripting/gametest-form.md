...
## 设置
与其他模块一样，你需要将依赖项添加到 `manifest.json` 中。
...
### ActionFormData
操作表单是包含多个按钮的表单。此表单非常适合商店用户界面、迷你游戏选择等。如果你看到一个特色服务器有很多按钮的用户界面，这就是该表单。
...
第二个参数是可选的，是按钮图标，显示在按钮上的图片/图标。要使用它，你需要定义纹理路径。你可以使用原版资源包来显示图标（例如 `textures/items/compass`）。自定义纹理需要在路径末尾加上 `.png`，并且在世界中需要有有效的资源包。
...
消息表单的主体文本限制非常大，因此你可以放入 5 行以上的文本。要添加新行，请使用 `\n`。
...
/// tip
由于消息表单只有 2 个按钮，建议在“按钮 2”上设置“是/确定”选项，在“按钮 1”上设置“否/取消”选项。你可以在“显示和响应”部分看到这个问题。
///
...
假设我们的表单必须通过一个名为“表单开启器”的棒子打开。你可以使用任何事件和任何配置来打开自己的表单。
...
/// warning
这些表单仅在没有其他用户界面打开时才能打开。如果你想通过自定义命令/聊天消息打开表单，则无法做到，因为聊天用户界面已打开。你需要使用 `/damage` 关闭聊天用户界面，然后打开表单。最佳选择是使用另一个事件。
///

在 if 语句内部是我们将显示表单的地方。使用 `.show()`，表单将打开。在 show 函数内部，你需要一个玩家类作为参数。显示表单后，我们可以使用 `.then()` 来保存玩家的响应。
...
当玩家关闭表单时，`.then()` 内的函数将运行，即使没有输入。这可能会导致玩家仅关闭表单时意外执行代码。为防止这种情况，你需要使用 `.canceled` 取消脚本。
...
### ActionFormData
操作表单将输入保存在 `.selection` 中。它返回按钮索引的数字，从 0 开始为按钮 1。你可以使用 switch-case 来为每个按钮运行特定的代码。
...
由于表单的输出基于顶部组件的顺序，你可以将每个输入分配给自己的变量。
