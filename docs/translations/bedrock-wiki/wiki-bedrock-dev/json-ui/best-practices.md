...
JSON-UI与所有其他附加包系统不同，因为**JSON-UI没有版本控制**。您对UI所做的任何更改可能会在Mojang更新和修复JSON-UI系统时出错。幸运的是，您可以采取几种方法来防止在Mojang更改原版UI时您的UI出错。

### 仅修改必要部分
最有效的最小化UI出错风险的方法是仅进行您需要的更改。例如，如果您只想禁用经验值栏阴影，您可能会认为应该将其添加到您的附加包中的`hud_screen.json`文件。
...
这在技术上是正确的，但容易出错，原因有几个：如果Mojang更改了绑定名称怎么办？如果他们更改了锚点怎么办？如果Mojang为元素添加了偏移怎么办？您所做的更改是与原版UI战略性地合并的，因此包含额外的细节是多余的，可能导致您的自定义UI出错。这可以简单地通过仅在元素中包含`shadow`属性来避免：
...
使用修改数组的修改策略是与原版UI战略性地合并，并且不会改变根面板中的兄弟控件。这样提高了与其他资源包的兼容性，并减少了您的UI出错的机会。
...
为了最佳修改此UI，您应尽可能避免嵌套树。例如，如果您想将标签颜色修改为灰色而不是白色，并将背景图像设为透明，请在元素定义中修改颜色和不透明度，而不是在树中修改（记住只修改必要的内容！）：
...
然而，有时无法避免修改树。在这种情况下，您应使用以下语法定位嵌套树中特定的控件。例如，要修改背景图像大小和标签层级，请使用以下语法。
...
`/`将定位到指定元素的子控件。请注意，如果指定的目标子控件名称不存在，将导致资源包错误。您的UI将正常运行，但由于此原因，最好尽量避免定位嵌套树中的控件。

### 利用单一入口点
为了向特定屏幕添加自定义UI，UI需要在某个点与原版UI合并。这称为入口点，最佳实践是在单一入口点将您的自定义UI与原版UI合并，以减少UI出错的可能性。以下是在`hud_screen.json`中使用两个入口点的示例：
...
在FPS方面，JSON-UI是第二耗费性能的附加包子系统，仅次于实体。您是否曾经想过为什么打开背包会将您的FPS减半？简短的回答是JSON-UI极其未优化，导致FPS降低。添加大量自定义UI可能会带来**显著的开销**，导致持续的游戏内帧率下降、屏幕加载时间变长以及整体糟糕的用户体验。
...
要解决此问题，您可以将其从UI树中删除，或在`panel`控件中添加`"ignored": true`。使用`"ignored": true`与删除它相同。控件及所有子控件将不会在UI中被评估，导致与其根本不存在时相同的性能效果。**使用**`"visible": false`**不会产生相同的效果，控件仍会被评估。**
...
有时，您可以简化并将多个控件合并为一个单一元素。例如，如果您想基于`#hud_title_text_string`是否为特定数字1-5来渲染特定的图像，您可能认为应该添加5个单独的控件来分别评估：
...
总的来说，为了提高UI的性能，了解您具体想要做什么，并尽可能按照这些通用指南进行调整是很重要的。保留单个控件未优化可能不会产生明显的差异。然而，当您开始保留许多未优化的控件时，性能可能会在已经未优化的UI系统上进一步下降。