# Minecraft基岩版文档仓库README译文

/// details-info | 来源信息
- 原文标题：README
- 原始仓库：<https://github.com/MicrosoftDocs/minecraft-creator>
- 对应来源：`.knowledge\Microsoft Learn\README.md`
///

本文为来源README的中文整理译文，保留原始信息顺序，并按本站术语习惯做最小化本地化处理。

## 概述

欢迎使用[Minecraft基岩版文档](https://learn.microsoft.com/minecraft/creator)。该文档站面向创作者，核心目标是帮助开发者通过附加包体系自定义Minecraft基岩版内容。

## 贡献

该仓库接受社区贡献与建议。新贡献者应先阅读官方贡献材料：

- [Contributing to Minecraft Bedrock Documentation](https://learn.microsoft.com/minecraft/creator/documents/contributorguide)
- [Contributor Style Guide](https://learn.microsoft.com/minecraft/creator/documents/styleguide)

若发现文档问题，可通过页面底部反馈入口或直接在仓库议题区提交问题：

- <https://github.com/MicrosoftDocs/minecraft-creator/issues>

来源说明指出，大多数贡献需要先同意贡献者许可协议（Contributor License Agreement），并在拉取请求流程中完成自动检查与确认。

## 开源行为准则

该仓库采用Microsoft开源行为准则（Microsoft Open Source Code of Conduct）。来源同时给出了常见问题页面与联系邮箱：

- <https://opensource.microsoft.com/codeofconduct/>
- <https://opensource.microsoft.com/codeofconduct/faq/>
- <mailto:opencode@microsoft.com>
