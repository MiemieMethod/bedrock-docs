---
title: LeviLamina文档归档
---

# LeviLamina文档归档

/// details-info | 来源信息
- 原文仓库：[github.com/LiteLDev/LeviLamina](https://github.com/LiteLDev/LeviLamina)
- 许可说明：以原仓库或原站点公开许可声明为准。
///



## 处理策略

- **站点入口**：文档站中的`Home`和`FAQ`分别保留为[文档首页](official-docs-home.md)和[常见问题](faq.md)，用于追溯站点门户、项目定位与更名缘由。
- **版本兼容矩阵**：官方导航末尾的`Supported Versions`保留为[支持的版本](versions.md)，用于追溯LeviLamina与BDS、客户端之间的兼容关系。
- **用户指南**：官方`User Guides`中的安装、Docker、客户端、问题排除与使用指南按原始顺序保留在本归档下。
- **开发者指南**：官方`Developer Guides`中的教程与`How-to Guides`主要并入[LeviLamina入门](../../guides/servers/levilamina.md)和[LeviLamina开发者指南](../../guides/servers/levilamina/index.md)；其中[官方项目架构](architecture.md)与[官方构建指南](build-guide.md)仍保留原始摘要。
- **API模块概览**：官方`API Reference`已并入[LeviLamina API模块](../../refs/server/levilamina-api.md)。
- **维护者指南**：官方`Maintainer Guides`保留为[C++风格指南](cpp-style-guide.md)。
- **仓库补充材料**：项目说明页、拉取请求模板、更新日志以及法律文件不属于文档站顶层导航，但对站内补全资料脉络仍有必要，故单独归档。
- **重叠信息分流**：文档首页与项目说明页都含项目定位和社区入口；前者主要保留站点门户与鸣谢语境，后者主要保留安装、贡献和许可证语境。
- **界面原文保留**：涉及LeviLauncher等工具界面时，保留原始按钮名称，例如“插件”“管理插件”；站内其他叙述仍统一使用“模组”。
- **原始脉络页**：保留为本归档下的独立条目，便于继续补充。

## 归档条目

### 站点入口

- [文档首页](official-docs-home.md)
- [常见问题](faq.md)

### 用户指南

- [在服务器上安装](install-server.md)
- [在Docker上安装](install-docker.md)
- [在客户端上安装](install-client.md)
- [问题排除](troubleshooting.md)
- [使用指南](usage-guidelines.md)

### 开发者指南

- [官方项目架构](architecture.md)
- [官方构建指南](build-guide.md)

### 维护者指南

- [C++风格指南](cpp-style-guide.md)

### 版本兼容

- [支持的版本](versions.md)

### 仓库补充材料

- [项目说明页](repository-readme.md)
- [拉取请求模板](pull-request-template.md)
- [更新日志](changelog.md)

### 法律与协议

- [法律与发布说明](legal-release-notes.md)
- [最终用户许可协议](eula.md)
- [行为准则](code-of-conduct.md)
- [许可证](license.md)

## 相关页面

- [LeviLamina](../../docs/server/levilamina.md)
- [LeviLamina客户端模组](../../docs/client/levilamina-client.md)
- [LeviLamina入门](../../guides/servers/levilamina.md)
- [LeviLamina开发者指南](../../guides/servers/levilamina/index.md)
- [LeviLamina API模块](../../refs/server/levilamina-api.md)
- [文档首页](official-docs-home.md)
- [常见问题](faq.md)
- [支持的版本](versions.md)
- [项目说明页](repository-readme.md)
- [拉取请求模板](pull-request-template.md)
- [法律与发布说明](legal-release-notes.md)
- [最终用户许可协议](eula.md)
- [行为准则](code-of-conduct.md)
- [许可证](license.md)
- [更新日志](changelog.md)